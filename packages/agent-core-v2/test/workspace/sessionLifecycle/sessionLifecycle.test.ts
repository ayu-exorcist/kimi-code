...
describe('SessionLifecycleService', () => {
  ...
  describe('fork session state', () => {
    ...
    it('loads persisted session language policy before returning from fork or resume', async () => {
      const root = await makeTmpRoot();
      const bootstrap = tmpBootstrapStub(root);
      const workDir = '/tmp/proj';
      const srcDir = join(root, 'sessions', 'wd_stub', 'src');
      const srcPolicy = join(srcDir, 'language-policy', 'state.json');
      ...
      const policy = {
        ...
        get ready(): Promise<void> {
          readyCount += 1;
          if (readyCount === 1) return Promise.resolve();
          const path = readyCount === 2 ? dstPolicy : srcPolicy;
          loaded.push(path);
          replyLanguage = undefined;
      ...
      const svc = await build([
        stubPair(IBootstrapService, bootstrap),
        stubPair(ISessionIndex, sessionIndexWithSummary('src', workDir, 'wd_stub')),
        stubPair(IAgentLifecycleService, agentLifecycleWithMainStub()),
        stubPair(ISessionLanguagePolicy, policy),
      ]);
      await svc.create({ sessionId: 'src', workDir });
      await mkdir(join(srcDir, 'language-policy'), { recursive: true });
      await writeFile(srcPolicy, '{"replyLanguage":"Japanese"}');
      ...
      const resumed = await svc.resume('src');

      expect(resumed?.accessor.get(ISessionLanguagePolicy).replyLanguage()).toBe('Japanese');
      expect(loaded).toEqual([dstPolicy, srcPolicy]);
    });
