...
describe('server-v2 /api/v1/config', () => {
  ...
  it('converts fields for a provider id named customBody without rewriting its custom_body value', async () => {
    await boot();
    await patchConfig({
      providers: {
        customBody: {
    ...
    const config = server!.core.accessor.get(IConfigService);
    await config.ready;
    const providers = config.get('providers') as Record<string, {
      baseUrl?: string;
      customBody?: Record<string, unknown>;
    }>;
    expect(providers['customBody']).toMatchObject({
